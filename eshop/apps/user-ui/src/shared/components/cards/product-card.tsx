...
import { useStore } from 'apps/user-ui/src/store'; // adjust this to match your actual store file path
...
const ProductCard = ({ product, isEvent }: { product: any; isEvent?: boolean }) => {
    ...
    return (
        <div className="w-full min-h-[350px] h-max bg-white rounded-lg relative">
            ...
            {product?.stock <= 5 && (
                <div className="absolute top-2 right-2 bg-yellow-400">
                    Limited Stock
                </div>
            )}
            <Link href={`/product/${product?.slug}`}>
                <img
                    src={product?.images[0]?.url || "https://ik.imagekit.io/eugenesokojamo/products/product-1787479615434_4jn1PmhYW.jpg?updatedAt=1787479618032"}
                    alt={product?.title}
                    width={300}
                    height={300}
            ...
            {isEvent && timeLeft && (
                <div className="mt-2">
                    <span className="inline-block text-xs bg-orange-100 px-2 py-1 rounded">
                        {timeLeft}
                    </span>
                </div>
            )}
