...
const ProductDetails = ({ productDetails }: { productDetails: any }) => {
    ...
    const [priceRange, setPriceRange] = useState([
        Math.max(0, (productDetails?.sale_price ?? 0) * 0.7),
        (productDetails?.sale_price ?? 1199) * 1.3,
    ]);
    const [recommendedProducts, setRecommendedProducts] = useState([]);

    const addToCart = useStore((store: any) => store.addToCart);
        ...
        <div className='w-full bg-[#f5f5f5] py-5'>
                    ...
                    <ReactImageMagnify
                        {...{
                            smallImage: {
                                ...
                                src:
                                    currentImage ||
                                    "https://ik.imagekit.io/eugenesokojamo/products/product-1787479615434_4jn1PmhYW.jpg?updatedAt=1787479618032"
                            },
                            largeImage: {
                                src:
                                    currentImage ||
                        ...
                        <div className="flex gap-2 overflow-x-auto">
                            {productDetails?.images?.map((img: any, index: number) => (
                                <Image
                                    ...
                                    width={60}
                                    height={60}
                                    className={`cursor-pointer border rounded-lg p-1 ${currentImage === img.url
                                            ? "border-blue-500"
                                            : "border-gray-300"
                                        }`}
                                    onClick={() => {
                                        ...
                                        {productDetails.colors.map((color: string, index: number) => (
                                            <button
                                                ...
                                                className={`w-8 h-8 cursor-pointer rounded-full border-2 transition ${isSelected === color
                                                        ? "border-gray-400 scale-100 shadow-md"
                                                        : "border-transparent"
                                                    }`}
            ...
            <div className="w-[90%] lg:w-[80%] mx-auto mt-5">
                <div className="bg-white min-h-[60vh] h-full p-5">
                    ...
                    <div
                        className="prose prose-sm text-slate-200 max-w-none break-words [overflow-wrap:anywhere]"
